...
main() {
  ...
  # ── 1. Intentar leer variables seteadas por Matugen ──────────────────
  thm_bar_bg="$(get_tmux_option "@thm_bar_bg" "")"
  thm_bg="$(get_tmux_option "@thm_bg" "")"
  thm_fg="$(get_tmux_option "@thm_fg" "")"
  thm_primary="$(get_tmux_option "@thm_primary" "")"
  thm_inverse_primary="$(get_tmux_option "@thm_inverse_primary" "")"
  thm_surface_low="$(get_tmux_option "@thm_surface_low" "")"
  thm_surface="$(get_tmux_option "@thm_surface" "")"
  thm_surface_variant="$(get_tmux_option "@thm_surface_variant" "")"
  thm_outline="$(get_tmux_option "@thm_outline" "")"
  thm_text_variant="$(get_tmux_option "@thm_text_variant" "")"

  # ── 2. Fallback: cargar el .tmuxtheme solo si Matugen no está activo ──
  if [ -z "$thm_bg" ]; then
    while IFS='=' read -r key val; do
      [ "${key##\#*}" ] || continue
      eval "$key"="$val"
    done < "${PLUGIN_DIR}/arrase.tmuxtheme"
  fi

  # ── 3. Mapping de variables (Matugen → nombres internos del tema) ─────
  [ -z "$thm_gray" ]   && [ -n "$thm_surface" ]         && thm_gray="$thm_surface"
  [ -z "$thm_gray" ]   && [ -n "$thm_surface_variant" ] && thm_gray="$thm_surface_variant"
  [ -z "$thm_gray" ]   && [ -n "$thm_outline" ]         && thm_gray="$thm_outline"
  [ -z "$thm_cyan" ]   && [ -n "$thm_primary" ]         && thm_cyan="$thm_primary"
  [ -z "$thm_blue" ]   && [ -n "$thm_primary" ]         && thm_blue="$thm_primary"
  [ -z "$thm_pink" ]   && [ -n "$thm_inverse_primary" ] && thm_pink="$thm_inverse_primary"
  [ -z "$thm_orange" ] && [ -n "$thm_inverse_primary" ] && thm_orange="$thm_inverse_primary"
  [ -z "$thm_black4" ] && [ -n "$thm_surface_low" ]     && thm_black4="$thm_surface_low"
  [ -z "$thm_fg" ]     && [ -n "$thm_text_variant" ]    && thm_fg="$thm_text_variant"
  [ -z "$thm_bg" ]     && [ -n "$thm_bar_bg" ]          && thm_bg="$thm_bar_bg"
  thm_green="$(get_tmux_option "@thm_green" "")"
  thm_red="$(get_tmux_option "@thm_red" "")"
  # ── 4. Aplicar opciones de tmux ───────────────────────────────────────
  ...
  local window_number_position=$(get_tmux_option "@window_number_position" "left")
  local window_status_enable=$(get_tmux_option "@window_status_enable" "no")
